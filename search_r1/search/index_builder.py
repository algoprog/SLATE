...
class Index_Builder:
    ...
    def __init__(
        ...
        self.faiss_metric = faiss_metric if faiss_metric is not None else "ip"
    ...
    @staticmethod
    def _resolve_metric(metric_name):
        if isinstance(metric_name, int):
            return metric_name
        if not isinstance(metric_name, str):
            raise ValueError("faiss_metric must be a string or faiss metric int constant.")
        normalized = metric_name.lower()
        if normalized in ("ip", "inner_product", "cosine"):
            return faiss.METRIC_INNER_PRODUCT
        if normalized in ("l2", "euclidean"):
            return faiss.METRIC_L2
        raise ValueError(f"Unsupported faiss metric: {metric_name}")
    ...
    def build_dense_index(self):
        ...
        if self.embedding_path is not None:
            embedding_source = self.embedding_path
        elif os.path.exists(self.embedding_save_path):
            embedding_source = self.embedding_save_path
            print(f"Found existing embeddings at {self.embedding_save_path}, skipping encoding.")

        if embedding_source is not None:
            all_embeddings = self._load_embedding(embedding_source, corpus_size, hidden_size)
        else:
        ...
        metric = self._resolve_metric(self.faiss_metric)
        faiss_index = faiss.index_factory(dim, self.faiss_type, metric)
...
def main():
    ...
    parser.add_argument('--faiss_metric', default='ip', choices=['ip', 'l2'], type=str)
