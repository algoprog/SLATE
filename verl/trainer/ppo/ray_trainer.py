...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # Debug logging
        print(f"[DEBUG] use_reference_policy: {self.use_reference_policy}")
        print(f"[DEBUG] ref_in_actor: {self.ref_in_actor}")
        print(f"[DEBUG] role_worker_mapping keys: {list(self.role_worker_mapping.keys())}")
        print(f"[DEBUG] use_kl_loss: {self.config.actor_rollout_ref.actor.use_kl_loss}")
        print(f"[DEBUG] kl_loss_coef: {self.config.actor_rollout_ref.actor.kl_loss_coef}")
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.use_reference_policy:
                        ...
                        with marked_timer(str(Role.RefPolicy), timing_raw, color="olive"):
                            ...
                            # Debug: log ref_log_prob stats
                            if "ref_log_prob" in batch.batch:
                                ref_lp = batch.batch["ref_log_prob"]
                                old_lp = batch.batch.get("old_log_probs", None)
                                if old_lp is not None:
                                    kl = (old_lp - ref_lp).abs().mean().item()
                                    print(f"[DEBUG] ref_log_prob mean: {ref_lp.mean().item():.4f}, old_log_prob mean: {old_lp.mean().item():.4f}, |KL|: {kl:.6f}")
                            else:
                                print("[DEBUG] WARNING: ref_log_prob not in batch after compute_ref_log_prob!")
