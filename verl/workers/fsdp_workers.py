...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_ref_log_prob(self, data: DataProto):
        print(f"[DEBUG] compute_ref_log_prob called: _is_lora={self._is_lora}, _is_ref={self._is_ref}, role={self.role}")
        if self._is_lora:
            # if _is_lora, actor without lora applied is the ref
            data.meta_info["is_lora"] = True
            data = self.compute_log_prob(data)
            # this old_log_probs is in fact ref_log_prob
            ref_log_probs = data.batch["old_log_probs"]
            print(f"[DEBUG] ref_log_prob computed via LoRA: shape={ref_log_probs.shape}, mean={ref_log_probs.mean().item():.4f}")
            data = DataProto.from_dict(tensors={"ref_log_prob": ref_log_probs})
            return data
        assert self._is_ref, f"Expected _is_ref=True but got {self._is_ref} with role={self.role}"
        # else:
