...
class vLLMHttpServerBase:
    ...
    async def generate(
        ...
        # Allow callers (e.g., agent loops) to request a smaller `max_tokens` for short "peek" generations.
        requested_max_tokens = sampling_params.pop("max_tokens", None)
        if requested_max_tokens is not None:
            if not isinstance(requested_max_tokens, int):
                raise TypeError(
                    f"Invalid max_tokens type: {type(requested_max_tokens)}; expected int|None"
                )
            if requested_max_tokens < 0:
                raise ValueError(f"max_tokens must be >= 0, got {requested_max_tokens}")
            max_tokens = min(max_tokens, requested_max_tokens)
        # vLLM `SamplingParams.logprobs` accepts either:
        # - None: disabled
        # - int: number of top logprobs to return (0 means only the sampled token)
        # Our higher-level code may pass a bool or an int; normalize here.
        _logprobs = sampling_params.pop("logprobs", None)
        if isinstance(_logprobs, bool):
            sampling_params["logprobs"] = 0 if _logprobs else None
        elif isinstance(_logprobs, int):
            sampling_params["logprobs"] = _logprobs
        elif _logprobs is None:
            sampling_params["logprobs"] = None
        else:
            raise TypeError(f"Invalid logprobs type: {type(_logprobs)}; expected bool|int|None")
        ...
        if sampling_params.logprobs is not None:
            ...
            # Convert vLLM Logprob objects to plain {token_id: logprob} dicts (for constrained sampling).
            top_logprobs = [
                {int(tok_id): float(lp.logprob) for tok_id, lp in step.items()}
                for step in final_res.outputs[0].logprobs
            ]
