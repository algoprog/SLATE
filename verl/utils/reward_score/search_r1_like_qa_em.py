...
def compute_score(solution_str, ground_truth, method="strict", format_score=0.0, score=1.0):
    ...
    # - Search-R1 format: {"target": [...]}
    # - Generic RL dataset format: ["a", "b"] or "a"
    if isinstance(ground_truth, dict):
        targets = ground_truth.get("target", None)
        if targets is None:
            targets = ground_truth.get("targets", None)
        if targets is None:
            # fall back to passing the whole dict through stringification
...
def compute_score_subem(solution_str, ground_truth, method="strict", format_score=0.0, score=1.0):
    ...
    # Support multiple ground truth formats (see compute_score()).
    if isinstance(ground_truth, dict):
        targets = ground_truth.get("target", None)
        if targets is None:
            targets = ground_truth.get("targets", None)
